...
The dataset below is built from a database of world cities, selected to have only cities of more than 5 million plus some cities in Canada. I included a maximum of 4 cities per country so that no one region of the globe was too strongly concentrated in points. You can get the data from [simplemaps](https://simplemaps.com/resources/free-country-cities) and make your own subset. I'll plot the data twice: once just as points in the plane, and once with a map of the major landforms of earth as a background.
...
```{r mds-map-cache, include=FALSE, message=FALSE, cache=TRUE, warning=FALSE}
WorldData <- map_data('world') 
ggplot() +
  geom_map(data = WorldData, map = WorldData, aes(map_id = region), fill = "white") +
  geom_point(data = cities, aes(x=lng, y=lat)) +
  geom_text_repel(data = cities, aes(x=lng, y=lat, label=city), size=2) 
```
...
```{r}
city_distance <- dist(cities |> dplyr::select(lng, lat) |> as.matrix())
  # distance in equirectangular projection
...
The dataset below was created as part of an experiment to measure the rate at which patterns of sounds for one symbol were confused with sounds for a different symbol. The matrix is symmetric dissimilarity measure. Rows and columns vary across the 36 symbols tested (26 letters and 10 numeric digits). All diagonals are 0s. The off diagonal values are large if the sounds are not likely to be confused. Smaller dissimilarities correspond to symbols that are more likely to be confused. A small excerpt of the table is shown.
...
You can also access the points from the ordination and make the plot using ggplot. The relative position of points on the plat is the only thing that matters -- any rotation or translation of the plot contains the same information.
...
In addition to `tidyverse`, `kableExtra` and `broom`, this lesson uses functions from the packages:

* `fields` (for computing a distance matrix)
* `vegan` (for the ordination functions)
* `ggmap` (for drawing maps)
* `ggrepel` (for labelling points on the maps)
