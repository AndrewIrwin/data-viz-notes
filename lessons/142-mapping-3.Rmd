...
A second approach is to group the rows and columns so that similar patterns are side by side. This can be accomplished with `tidyheatmap` and using the `cluster_rows` and `cluster_cols` options.
...
Change the `scale` option to `row`. Make a guess about what the heatmap will show before you draw the figure.
...
* `cansim` for Statistics Canada data
* `tidyheatmaps` 
* `statebins` for drawing maps with one block per state

## Further reading

* Healy [@healy2018data] Chapter 7 [Maps](https://socviz.co/maps.html)
