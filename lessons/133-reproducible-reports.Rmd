...
We have been using R packages and organizing all files for a project in a folder (and sub-folders) created just for that project. This is a good first step. The `here` package provides a useful function that allows you refer to a file relative to the directory where your `.Rproj` file is stored. This is [important](https://malco.io/2018/11/05/why-should-i-use-the-here-package-when-i-m-already-using-projects/) to make your R code work on someone else's computer. Here's how you use it:
    ...
    my_data <- read_csv(here("data", "annual-number-of-deaths-by-cause.csv"))

This tells R to find a folder called `data` in your project, then look for a file called `annual-number-of-deaths-by-cause.csv` in that folder. If this code works on your computer, with your R project, and you give the whole folder to a collaborator, you don't need to worry how they organize their files.
...
-   Use bulleted or numbered lists (start a sequence of lines with a `*` or `1.`) when appropriate.
-   Format your code nicely. Use a [style guide](https://style.tidyverse.org/). Or use an automatic code formatter such as styler (see below). Leave a blank line before and after your code chunks.
...
For examples of these formatting tips, see the [source for this file](https://github.com/AndrewIrwin/data-viz-notes/blob/main/lessons/133-reproducible-reports.Rmd).
...
gapminder |> filter(continent =="Europe") |> group_by(country) |>
  summarize(mean_life_exp = mean(lifeExp), mean_GDP = mean(gdpPercap)) |>
  ggplot(aes(color = mean_life_exp, x = log10(mean_GDP), y=fct_reorder(country, mean_life_exp))) + geom_point()
```
...
## Customization of result reporting in quarto

The first line of a code chunk can be as simple as `{r}` but you can also include many options between the braces or in special comments in the code block. The [quarto documentation](https://quarto.org/docs/computations/r.html) and [markdown book](https://bookdown.org/yihui/rmarkdown-cookbook/chunk-options.html) describe the options available. Here I will demonstrate a few chunk options related to figures and formatting output.


Rendering (quarto, or knitting for Rmd) a document usually stops when an error is encountered. This is a safety measure to alert you to a document which is not reproducible because of errors. On rare occasions you may want to use the chunk option `error=TRUE` to allow error messages to appear in knitted output and not stop the knitting process.
...
Quarto allows you to combine several different programming languages, including python and Julia in the same document. For more information see the [Quarto](https://quarto.org/) website. 
...
[SQL](https://en.wikipedia.org/wiki/SQL) is a language for describing queries to a widely used style of database. R contains tools for interacting with SQL databases, but it can also generate SQL code from `dplyr` functions.
Here's an example showing how R can generate SQL queries from dplyr calculations.
...
What does using other tools have to do with making your work reproducible? Quarto is a flexible tool that lets you use more than just R, so if your workflow contains steps external to R, they can sometimes still be included in your report.

## Packages used

In addition to `tidyverse` and `gapminder`, code in this lesson uses the packages

* `report` which is availbale on github and can be installed using `remotes::install_github("easystats/report")` for the `cite_packages` function,
* `here` to help find files in a project, even when you move the project to a new computer,
* `styler` for formatting R code,
* `dbplyr` and `RSQLite` for working with databases.

## Further reading

-   [Repository of these course notes](https://github.com/AndrewIrwin/data-viz-notes) for ideas on formatting and preparing R markdown documents
-   [Project workflow](https://r4ds.hadley.nz/workflow-scripts.html) from R4DS
...
-   LeVeque et al. [Reproducible research for scientific computing](https://www.bitss.org/wp-content/uploads/2014/06/reproducible-research-for-scientific-computing-tools-and-strategies-for-changing-the-culture1.pdf) [@leveque2012]
