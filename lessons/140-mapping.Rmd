...
In addition to `tidyverse` this lesson uses the following packages:

* `ggmap` to get data for maps and draw them
* `mapproj` to project latitude and longitude coordinates onto a map
* `sf` for working with map data in data tables
* `geojsonio` for reading a map of Canada from the internet
* `rmapshaper`
