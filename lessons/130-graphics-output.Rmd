...
library(svglite)
library(ragg) # May require github version: devtools::install_github('r-lib/ragg')
```
...
For raster images, the the "dots per inch" (dpi) provides some information on how the figure is intended to be displayed. Monitors, phones, and other devices generally range from 100-250 dpi and physical printing usually ranges from 300-600 dpi. There are, of course, exceptions.
...
```{r warning = FALSE, fig.width = 6, fig.height= 4, fig.align="center", dev=c("png", "pdf"), out.width = "50%"}
p1
```

## Vector or raster?

The quality of a vector image (PDF, svg) should always be superior to a raster image (png). Vector images can be redrawn at different scales easily, making it possible for your reader to zoom in on your graphics. If your plot has millions of points or a huge number of lines, then a vector format which requires the computer to draw each feature every time the image is shown will be slow. So you should normally use a vector format (svg for the web, pdf for printing) and only use raster formats (png) for very complicated figures, for example if you notice that your graphics are displaying slowly.
...
The `ragg` library and graphics output device also provides reliable rendering text in many languages and a way to scale the size of images across different output formats. I had 
an example with Japanese text and emoji, but it seems to not be working properly at the moment, so I'm showing the map without those text elements.

For more information see [this tutorial](https://www.tidyverse.org/blog/2021/02/modern-text-features/).

```{r echo=FALSE, dev='ragg_png'}
main_cities <- data.frame(
  name = c("Tokyo", "Yokohama", "Osaka", "Nagoya", "Sapporo", "Kobe", "Kyoto", "Fukuoka",
  "Kawasaki", "Saitama"),
...
ggplot() + 
  ...
  ggrepel::geom_label_repel(
    ...
    point.padding = grid::unit(1, "mm"),
    max.overlaps = 15,
    size = 2) +
  geom_point(aes(lon, lat), main_cities) +
  ggtitle(
    "Location of largest cities in Japan"
  ) +
  theme_void() + 
