...
* Ensure the computer helps you extend and expand your knowledge, rather than just replacing your brain (the GPS problem), and
* Develop strategies to determine if the solutions provided by the computer are correct.
...
Over the past few months I have made an effort to get various LLM (chat gpt, copilot, gemini, deepseek) to help me with data analysis and visualization tasks by writing computer code to perform the following tasks:

* generate points along a [geodesic](https://en.wikipedia.org/wiki/Geodesics_on_an_ellipsoid) to plot a path on a map,
* parse a [AIS messages](https://en.wikipedia.org/wiki/Automatic_identification_system) from commercial ships, which are encoded in a complicated, nested data structure,
* compute prime numbers using a [sieve wheel](https://en.wikipedia.org/wiki/Sieve_of_Pritchard),
* write a complicated statistical model in a [probabilistic programming language](https://en.wikipedia.org/wiki/Probabilistic_programming), and
* assign students to projects based on a set of [preference scores](https://en.wikipedia.org/wiki/Assignment_problem).
...
Since we're in the winter freeze-thaw snow-rain-ice cycle right now, my mind turned to the following question: _tidyverse code to compute the distribution of the number of days in a row with a daily high temperature below freezing in Halifax_. Gemini did a good job with this task.
